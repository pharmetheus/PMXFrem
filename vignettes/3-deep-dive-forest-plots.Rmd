...
formatted_row_labels <- generateCovNames(dfCovs, unit_map = unit_map, label_map = label_map)

# 3. Apply labels to the dataset
dfres_formatted <- dfres %>% 
  mutate(COVNAME = factor(rep(formatted_row_labels, each = length(unique(PARAMETER))), 
                          levels = formatted_row_labels))

# 4. Define group headers (y-axis facets)
group_headers <- c(

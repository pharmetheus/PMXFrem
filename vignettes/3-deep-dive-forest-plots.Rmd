...
**Handling Missing Covariates:** You must build defensive fallback logic for missing covariates (e.g., `dfrow$FOOD != -99`) *only* for covariates outside the FREM part of the model (FFEM covariates). `PMXFrem` automatically handles missing data imputation for the covariates included in the FREM covariance matrix. However, because the `dfCovs` matrix intentionally injects `-99` to isolate covariate effects, your function will encounter missing data for FFEM covariates like `FOOD` and must know how to revert to the baseline fixed effect.

...
# Calculate ETAs, ETA Prims, and EBEs. With the FREM model located through
# `runno` / `modDevDir`, `numSkipOm` and `numNonFREMThetas` are derived from
# the model and its `.ext` (via `fremModelInfo()`); pass them only to override.
ind_params <- calcEtas(
  runno              = fremRun,
  modDevDir          = modDevDir,
  dataFile           = clin_data,

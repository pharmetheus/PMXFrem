...
Similarly to the standar diagnostics variables (e.g. CWRES) ETAs will also have to be adjusted to reflect the impact of covariates, i.e. the "ETA prims" needs to be computed. This is done by the `calcParameters` function. This function requires an "FFEM data object" from the `createFFEMmodel` function as input.
...
```{r}
ind_params <- calcParameters(runno            = fremRun,
                             modDevDir        = modDevDir,
                             numSkipOm        = 2,
                             numNonFREMThetas = 7,
                             FFEMData         = FFEMdata) %>% 

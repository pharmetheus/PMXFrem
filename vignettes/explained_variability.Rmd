...
Collect the file names related to the FREM run we are visualizing the results from. The utility function `getFileNames` returns a liss of file paths for the `.mod`, `.ext`, `.lst`, `.phi` and `.cov` files.
```{r} 
...
The function `getExplainableVarDF`computes the total and explained variability. This can be done in 4 different ways, specified with the `type`argument.
...
```{r}
dfres  <- getExplainableVarDF(type             = 2,
                              data             = dfData,
                              dfCovs           = dfCovs,
...
The `plotExplainedVar` function can be used to visualize the degree of explained variability. The plot can visualise the explained variability relative the `TOTVAR`column (`type=1`), how much of the overall variability can be explained, or relative to `TOTCOVVAR` (`type=2`), i.e. how much of the explainable variability that can be explaines.
...
plotExplainedVar(dfres,parameters = "AUC",type = 1,covariateLabels = covariateNames)
```

```{r fig.width=10,fig.height=10}
covariateNames <- c("ALL","Age","ALT","AST","Bilirubin","BMI","BSA","CRCL","Ethnicity","Genotype","Height","LBWT","NCI","Race","Sex","Smoking","WT","Food status")
plotExplainedVar(dfres,parameters = "AUC",type = 2,covariateLabels = covariateNames)
```

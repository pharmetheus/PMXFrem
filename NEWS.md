...
* **ETA Mismatch in EV:** Fixed longitudinal subsetting (`!duplicated(strID)`) to prevent ETA length mismatches in `getExplainedVar()`.
* **Missing Covariate Failsafe:** Added explicit `stop()` errors for missing categorical covariates in `getExplainedVar()`.
* **NSE Scoping in data.table:** Fixed `j symbol` scoping errors by coercing inputs to base `data.frame` in variance calculations.

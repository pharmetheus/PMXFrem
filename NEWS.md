...
## New Features
* **`createFREMParamFunction()` / `verifyFREMParamFunction()` — generate a FREM parameter function from the base model.** `createFREMParamFunction(baseModel, parameters, numSkipOm)` translates the base (pre-FREM) model's `$PK` into R source for a `function(basethetas, covthetas, dfrow, etas, ...)`: the structural `$PK` (including non-FREM covariates such as allometric weight) is transliterated, and for each named parameter its single `ETA()` reference is replaced **in place** - whatever encloses it - by `covthetas[k] + etas[numSkipOm + k]` (the FREM covariate coefficient is additive on the eta scale). A named parameter with no `ETA()` in `$PK`, or more than one, is an error. One generated function serves both plots -- call it with `etas = 0` for a `getForestDFFREM()` forest plot, with non-zero `etas` for a `getExplainedVar()` explained-variability plot. Nothing is evaluated; the source is returned for review. `verifyFREMParamFunction()` checks a generated function three ways: the structural part matches `PMXForest::createParamFunction()`'s typical values, `covthetas[k]` scales parameter `k` by `exp(covthetas[k])`, and `etas[numSkipOm + k]` scales it by `exp(eta)`. Built on `PMXForest::nmParsePK()` (requires PMXForest >= 1.2.15.9005).

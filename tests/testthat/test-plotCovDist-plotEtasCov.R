...
test_that("plotCovDist errors when a requested covariate is absent", {
  expect_error(
    plotCovDist(indParams, covNames = c("WT", "NOPE")),
...
test_that("plotCovDist warns and treats all as non-missing when flags are absent", {
  ...
  expect_warning(
    p <- plotCovDist(noFlags, covNames = "WT"),
...
test_that("plotEtasCov returns a ggplot with FREM + Prim facet rows by default", {
  ...
  expect_setequal(
    as.character(unique(built$plot$data$Type)),
    c("FREM ETA", "ETA Prim")
  )
})
...
test_that("plotEtasCov can plot the EBE type and respects row order", {
  p <- plotEtasCov(indParams,
    covName = "WT", etaNames = c("ETA3", "ETA4"),
    etaTypes = c("EBE", "FREM"),
...
test_that("plotEtasCov applies custom etaLabels", {
  ...
  expect_setequal(
    levels(ggplot2::ggplot_build(p)$plot$data$ETA),
    c("Clearance", "Volume")
  )
})

test_that("plotEtasCov adds a missing-data smooth layer when asked", {
  pNo <- plotEtasCov(indParams,
    covName = "WT", etaNames = "ETA3",
    etaTypes = "FREM"
  )
  pYes <- plotEtasCov(indParams,
    covName = "WT", etaNames = "ETA3",
...
test_that("plotEtasCov excludes missing subjects unless showMissing = TRUE", {
  pStrict <- plotEtasCov(indParams,
    covName = "WT", etaNames = "ETA3",
    etaTypes = "FREM"
  )
  pAll <- plotEtasCov(indParams,
    covName = "WT", etaNames = "ETA3",
...
test_that("plotEtasCov errors when the covariate is missing", {
  expect_error(
    plotEtasCov(indParams, covName = "NOPE"),
    "'NOPE' not found"
  )
})
...
test_that("plotEtasCov errors on an unknown requested ETA", {
  expect_error(
    plotEtasCov(indParams, covName = "WT", etaNames = "ETA99"),

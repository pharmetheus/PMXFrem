test_that("the correct columns are added", {
  # Ensure missVal is explicitly defined in the local test environment
  ...
  data_missing <- data
  data_missing$RACEL[1:2] <- missVal

  # Test imputeMissing = FALSE (FREM behavior: preserves -99)
  newData_frem <- addFREMcovariates(data_missing,
    covariates = c("RACEL"),
    missVal = missVal, imputeMissing = FALSE
  )

  expect_equal(newData_frem$RACEL_3[1:2], c(missVal, missVal))
  expect_equal(newData_frem$RACEL_2[1:2], c(missVal, missVal))

  # Test imputeMissing = TRUE (FFEM behavior: imputes to 0, which is the default)
  newData_ffem <- addFREMcovariates(data_missing,
    covariates = c("RACEL"),
    missVal = missVal, imputeMissing = TRUE
  )

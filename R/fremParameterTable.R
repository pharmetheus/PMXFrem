...
#'   numSkipOm <- 2
#'
#'   ## numNonFREMThetas / numSkipOm omitted: derived from the model located via
#'   ## runno / modDevDir (see fremModelInfo())
#'   tmpDerived <- fremParameterTable(
#'     runno     = runno,
#'     modDevDir = modDevDir,
#'     thetaNum  = 2:7,
#'     omegaNum  = c(1, 3, 4, 5),
...
#'   )
#'
#'   ## Full output, numNonFREMThetas / numSkipOm passed explicitly
...
#' @concept diagnostics
fremParameterTable <- function(runno = NULL,
                               modDevDir = NULL,
  ...
  ffem_point <- calcFFEM(
    dfext = extRes, numNonFREMThetas = numNonFREMThetas, numSkipOm = numSkipOm,
  ...
  if (includeRSE) {
    ...
    for (i in seq_len(nrow(dfSamplesBS))) {
      ...
      # Sample Coefficients
      tmp_ffem <- calcFFEM(
        dfext = dfSamplesBS[i, , drop = FALSE], numNonFREMThetas = numNonFREMThetas,
  ...
  if (includeShrinkage) {
    ...
    if (!(shrinkageType %in% names(shk_df))) {
      stop(sprintf(
        "Invalid shrinkageType '%s'. Valid options are: %s",

...
createFREMmodel <- function(runno = NULL,
  ...
  if (file.exists(finalModelPath) || file.exists(finalDataPath)) {
    stop(
      sprintf(
        "Protection Error: The output files '%s' or '%s' already exist in the target directory (%s). Please use a different `fremModName`, specify a new `outputDir`, or manually remove the existing files.",
  ...
  if (is.null(numNonFREMThetas)) {
    ...
    numNonFREMThetas <- length(grep(
      "^THETA",
      names(getExt(extFile = fileNames$ext))
    ))
    if (!quiet) {
      message(
        "createFREMmodel(): numNonFREMThetas = ", numNonFREMThetas,

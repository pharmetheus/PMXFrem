...
#' @param numSkipOm Number of diag omegas (variances) that should not be part of the FREM calculations. 
#' Such omegas has to come before the large FREM omega block.
#' @param numFREMThetas  The number thetas associated with FREM covariate effects. 
#' @param numSigmas Number of sigma parameters in the ext file model (including zero sigma covariance elements).
...
calcFFEM <- function(dfext,numNonFREMThetas,numSkipOm=0,numFREMThetas=length(grep("THETA",names(dfext)))-numNonFREMThetas,
  ...
  #df_om  <- as.numeric(dfext[,(numNonFREMThetas+5+numFREMThetas):(numNonFREMThetas+4+numFREMThetas+iNumFREMOM)])

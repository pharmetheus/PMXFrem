...
createFFEMdata2 <- function(runno = NULL,
                           numNonFREMThetas,
                           modName       = NULL,
  ...
  myFun <- function(data, parNames, dataMap = dataMap, availCov = NULL, covSuffix) {
    ...
    # ----------------------------------------------------------------------
    # NEW: Extract the variance/covariance matrix as V-columns with offset
    # ----------------------------------------------------------------------
    myOmega <- ffemObj$FullVars
    
    # Remove skipped omegas to align with the parameter list
    if (numSkipOm != 0) {

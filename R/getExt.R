...
getExt <- function(extFile, set = NULL) {
  ...
  if (set == 1 & length(tabs) == 1) { # Only one set of results
    myext <- read.table(extFile, skip = 1, h = TRUE)
  } else if (set == 1 & length(tabs) > 1) {
    myext <- read.table(extFile, skip = 1, nrows = tabs[2] - 3, h = TRUE)
  } else if (set == 2 & length(tabs) == 2) {
    myext <- read.table(extFile, skip = tabs[2], h = TRUE)
  } else if (set == 2 & length(tabs) == 3) {
    myext <- read.table(extFile, skip = tabs[2], nrows = length(tmp) - tabs[2] - (length(tmp) - tabs[3]) - 2, h = TRUE)
  } else if (set == 3 & length(tabs) == 3) {
    myext <- read.table(extFile, skip = tabs[3], h = TRUE)
  } else if (set == 4 & length(tabs) == 4) {
    myext <- read.table(extFile, skip = tabs[4], h = TRUE)
  }

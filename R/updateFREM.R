...
updateFREM <- function(strFREMModel,
  ...
  if (strUpdateType == "NoData" || strUpdateType == "NewInits") {
    if (!file.exists(paste0(file_path_sans_ext(strFREMModel), ".ext")))
      stop(paste0("NoData and NewInits demands that ", paste0(file_path_sans_ext(strFREMModel), ".ext"), " exists"))
  }
  ...
  if (file.exists(paste0(file_path_sans_ext(strFREMModel), ".ext"))) {
    ...
    dfext     <- subset(getExt(extFile = paste0(file_path_sans_ext(strFREMModel), ".ext")), ITERATION == "-1000000000")
    # Read in which parameteras that are fixed
    dfextfix  <- subset(getExt(extFile = paste0(file_path_sans_ext(strFREMModel), ".ext")), ITERATION == "-1000000006")
    ...
    if (file.exists(strFREMModel)) {
      ...
      ### Get the maximum THETA number, i.e. number of THETAs in model
      for (str in os) {
        tmp                                        <- gsub(".*THETA\\(([0-9]+)\\).*", "\\1", str)
        if (as.numeric(tmp) > iNumTHETA) {
          iNumTHETA <- as.numeric(tmp)
        }
      }
      ...
      ## Get the maximum OMEGA number, i.e. number of ETAs in model
      for (str in os) {
        tmp <- gsub(".*[ ]ETA\\(([0-9]+)\\).*", "\\1", str)
        if (as.numeric(tmp) > iNumOM) {
          iNumOM <- as.numeric(tmp)
        }
      }

#' setupdfCovs
#'
#' @description Create the dfCovs data.frame that is an argument to the `getExplainedVar`.
#' function.
#' @param modFileName A path to the FREM model file.
#' @param fremCovs A vector of covariates that are part of the FREM definition of the FREM model file.
#' Default is all, i.e. `getCovNames(modFile = modFileName)$orgCovNames`
#' @param additionalCovs Any additional covariates to be included in the output dfCovs. For example
#' covariates that are part of the fixed effects part of the FREM model file.
#' @return A data.frame that can be used as the dfCovs argument to `getExplainedVar`.
#' @export
#'
#' @examples
#'

...
calcFFEM <- function(noBaseThetas,noCovThetas,noSigmas,noParCov=noBaseThetas,noSkipOm=0,dfext,
                     ...
                     availCov=covNames,quiet=FALSE,fremETA=NULL) {
  ...
  if (!is.null(fremETA)) {
    if (noSkipOm==0) {
      if (!exists("missCov")) eta_prim<-fremETA[1:noParCov]-COEFF%*%fremETA[(noParCov+1):(noParCov+noCovThetas)]
      else eta_prim<-fremETA[1:noParCov]-COEFF%*%fremETA[(noParCov+1):(noParCov+noCovThetas)][-missCov]
    } else {
      if (!exists("missCov")) eta_prim<-c(fremETA[1:noSkipOm],fremETA[(noSkipOm+1):(noParCov+noSkipOm)]-COEFF%*%fremETA[(noSkipOm+noParCov+1):(noSkipOm+noParCov+noCovThetas)])
      else eta_prim<-c(fremETA[1:noSkipOm],fremETA[(noSkipOm+1):(noParCov+noSkipOm)]-COEFF%*%fremETA[(noSkipOm+noParCov+1):(noSkipOm+noParCov+noCovThetas)][-missCov])
    }
  }

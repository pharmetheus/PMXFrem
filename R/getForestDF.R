...
#' }
getForestDF <- function(dfCovs,functionList=list(function(thetas,coveffects,...){return(thetas[1]*exp(coveffects[1]))}),functionListName="PAR1",noBaseThetas, noCovThetas, noSigmas, noParCov = noBaseThetas, dfParameters,
                        parNames = paste("Par", 1:noParCov, sep = ""), covNames = paste("Cov",1:noCovThetas, sep = ""),
                        availCov = covNames, quiet = FALSE,probs=c(0.025,0.5,0.0975),dfRefRow=NULL) {
  ...
  for (i in 1:nrow(dfCovs)) {
    for (j in 1:length(functionList)) {
      ...
      dfrow<-cbind(dfCovs[i,],data.frame(COVNUM=i,PARAMETER=functionListName[j],REFMEAN=mean_base,REFTRUE=true_base,REFMEDIAN=median_base))

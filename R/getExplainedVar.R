...
#' @param functionList A list of functions with inputs (basethetas, covthetas,
#'   dfrow, etas, ...) for which the explained variability will be calculated,
#'   or a single such function. A function may return a vector of values; each
#'   value is used, and functionListName must then name every value returned by
#'   every function, in order. A function must return the same number of values
#'   for every row: in a `dfCovs` covariate row, `dfrow` holds only that row's
#'   covariates, so a function that reads another column (for example
#'   `dfrow$FOOD`) gets `NULL` there and has to test for the column first.
#' @param functionListName A vector of strings (names) for the parameters generated by functionList.
#' @param numNonFREMThetas Number of structural thetas in the FREM model. If
#'   `NULL` (default) it is derived from the model file and the ext via
#'   [fremModelInfo()]; a supplied value that disagrees with the derived one
#'   triggers a warning and is kept.
...
#'   covariates, i.e. skip before calculating the FREM variances. If `NULL`
#'   (default) it is derived via [fremModelInfo()]; a supplied value that
#'   disagrees with the derived one triggers a warning and is kept.
...
#'   type=1: one row per individual in `data`, in the order in which each
#'   individual first appears in `data`. If `NULL` when type=1 the etas are read
#'   from the phi-file and matched to `data` by `strID`.
...
#' @param seed Used when sampling ETAs in type==2 and type==3. The default,
#'   `NULL`, sets no seed, so the samples follow the current random number
#'   state; pass an integer for reproducible results.
#' @param missVal Numeric. The value representing missing data in the covariates. Defaults to -99.
#' @param ... additional arguments forwarded to the functions in
#'   `functionList`. `modExt` and `lstExt` are also passed on to
#'   [getFileNames()].
#'
...
#' of the maximum variability (TOTCOVVAR). If NULL (default), all covariates in the frem model will be used for the derivation of TOTCOVVAR.
#' A multi-level categorical covariate may be given by its original name (`RACEL`) or by its binarized FREM
#' names (`RACEL_2`, `RACEL_3`); a name that is neither is an error.
#'
#' @examples
#' \donttest{
#' library(dplyr)
...
getExplainedVar <- function(type = 1,
  ...
  if (is.function(functionList)) functionList <- list(functionList)

  if (type > 0 && !is.null(data)) {
    data <- as.data.frame(data)
  }

  ## `...` is for the parameter functions; only getFileNames()'s own
  ## extension arguments are routed to it, so any other argument no longer
  ## makes getFileNames() fail with "unused argument".
  dots <- list(...)
  fileNames <- do.call(getFileNames, c(
    list(runno = runno, modName = modName, modDevDir = modDevDir),
    dots[names(dots) %in% c("modExt", "lstExt")]
  ))
  ...
  ## numParCov is needed by every type, whether or not parNames is given
  ...
  if (is.null(parNames)) {
    parNames <- paste("Par", seq_len(numParCov), sep = "")
  }

  etaIDs <- NULL
  if (type == 1 && is.null(etas)) {
    dfPhi <- getPhi(phiFile)
    etas <- dfPhi[, 3:(2 + numParCov + numSkipOm), drop = FALSE] # Include the structural model etas only
    etaIDs <- dfPhi$ID
  }
  ...
  if (is.null(availCov)) {
    ...
    ## Expand an original categorical name (RACEL) to its binarized FREM
    ## columns (RACEL_2, RACEL_3). Matched exactly, an original name was
    ## silently left out of TOTCOVVAR.
    allCov <- .get_frem_cov_names(availCov, fremCovs)
    unknown <- setdiff(allCov, covNames)
    if (length(unknown) > 0) {
      stop("availCov: ", paste(unknown, collapse = ", "), " is not a FREM ",
        "covariate of ", basename(modFile), ". Available: ",
        paste(orgCovs, collapse = ", "), ".",
        call. = FALSE
      )
    }
  }
  ...
  if (type %in% c(1, 2, 3)) {
    return(.calc_empirical_variance(
      ...
      parNames = parNames, numSkipOm = numSkipOm, allCov = allCov, etas = etas, etaIDs = etaIDs, quiet = quiet, ncores = ncores,

...
getPhi <- function(phiFile, set = NULL, warn = TRUE) {
  ...
  if (set == 1 & length(tabs) == 1) { # Only one set of results
    myphi <- read.table(phiFile, skip = 1, header = TRUE)
  } else if (set == 1 & length(tabs) > 1) {
    myphi <- read.table(phiFile, skip = 1, nrows = tabs[2] - 3, header = TRUE)
  } else if (set == 2 & length(tabs) == 2) {
    myphi <- read.table(phiFile, skip = tabs[2], header = TRUE)
  } else if (set == 2 & length(tabs) == 3) {
    myphi <- read.table(phiFile, skip = tabs[2], nrows = length(tmp) - tabs[2] - (length(tmp) - tabs[3]) - 2, header = TRUE)
  } else if (set == 3 & length(tabs) == 3) {
    myphi <- read.table(phiFile, skip = tabs[3], header = TRUE)
  } else if (set == 4 & length(tabs) == 4) {
    myphi <- read.table(phiFile, skip = tabs[4], header = TRUE)
  }
